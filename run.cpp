...
{
    vector<int>::iterator max = max_element(pos.begin(), pos.end());
    if (mode == H_PROJECT)
    {
        int height = pos.size();
        int width = *max;
...
void OCRecognizer::run() {
    ...
    Net<float> lenet(this->modelpath0, caffe::TEST);//????1
    lenet.CopyTrainedLayersFrom(this->modelpath1);//????2
...
void OCRecognizer::set_modelpath0(char *p){
    strcpy(this->modelpath0, p);
}

void OCRecognizer::set_modelpath1(char *p){
    strcpy(this->modelpath1, p);
}
