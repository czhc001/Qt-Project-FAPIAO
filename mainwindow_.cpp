    ...
    ui(new Ui::MainWindow)
{
    ...
    ui->label_camera_alert->setVisible(false);
...
void MainWindow::initializa_UI(){
    ...
    ui->label_camera_alert->setPalette(palette1);
    ui->label_camera_alert->setFont(font);
    ui->label_camera_alert->setText(QString::fromLocal8Bit("  摄像头未连接，请连接后点击 开启摄像"));
...
void MainWindow::on_Error_Message(int a){
    if(ui->label_image->text().size() > 0)
        ui->label_image->clear();
    ui->label_camera_alert->setVisible(true);
                ...
                if(Object.contains("data")){
                    ...
                    if(dataObj.isBool()){
                        ...
                        if(QString::compare("norule", dataStr, Qt::CaseSensitive) == 0){
                            ...
                            YesruleDialog *dialog = new YesruleDialog(userid, versionid, current_code, current_no, this->manager_name,
                                                                      this->department, this->current_time.toString("yyyy/MM/dd"), this->serial_number,
                                                                      this->username, history_code, history_no, history_manager, history_department, history_datetime.toString("yyyy/MM/dd"),
                                                                      history_serial, history_username, this);
